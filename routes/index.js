...
router.get('/api/active', fetchTodos, async function(req, res, next) {
  const apikey = req.query.apikey;
  auth_res = await auth.verifyApiKey(apikey);
  console.log("auth: "+ auth_res.authenticated)

  if(auth_res.authenticated != true) {
    res.status(403).send({message : auth_res.message})
  } else {
    res.json(res.locals.todos.filter(function(todo) { return !todo.completed; }));
  }
