...
function verifyApiKey(apikey) {
  return new Promise((resolve) => {
    test = db.get('SELECT * FROM apikeys WHERE key = ?', [ apikey ], function(err, row) {
      console.log("err:" + err)
      console.log("row:" + row)
      if (err) { return  resolve({authenticated: false , err: err}); }
      if (!row) { return resolve({authenticated: false, message: 'Incorrect apikey.' }); }
